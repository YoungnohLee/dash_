...
tools = [
    ...
    },
    {
        ...
        "function": {
            "name": "google_search",
            "description": "Use this function to perform a Google search and get the top search results.",
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {
            ...
            for choice in response.choices:
                ...
                if choice.message.tool_calls:
                    for tool_call in choice.message.tool_calls:
                        ...
                        elif function_name == "google_search":  # google_search 호출
                            query = function_arguments["query"]
                            search_results = google_search.google_search(query)
                            if search_results:
                                for result in search_results:
                                    conversation_stack.append(html.Div([
                                        html.H3(result["title"]),
                                        html.A(result["link"], href=result["link"], target="_blank"),
                                        html.P(result["snippet"]),
                                        html.Hr()
                                    ]))
                            else:
                                conversation_stack.append(html.P("No results found."))
                        else:
